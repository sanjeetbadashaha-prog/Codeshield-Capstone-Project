...
def fetch_github_files(repo_url: str) -> list:
    """Parses a GitHub repo URL, fetches all .py files, and returns their raw content.
    ...
    try:
        ...
        # Iterate and fetch raw content of python files
        for entry in tree:
            path = entry.get("path", "")
            entry_type = entry.get("type", "")
            
            # Filter only .py files and ignore folders (blobs only)
            if entry_type == "blob" and path.endswith(".py"):
